...
public class GeminiFeignClient {
    ...
    @Value("${feign.api.gemini.url}")
    private String apiUrl;

    private final ObjectMapper objectMapper;
    private final OkHttpClient okHttpClient;

    public GeminiFeignClient(ObjectMapper objectMapper) {
     ...
     * 流式调用Gemini API
     */
    public void streamCompletion(GeminiRequest request, String apiKey, StreamCallback callback) {
        ...
        try {
            ...
            log.info("调用Gemini API: {}", requestBody);

            // 构建请求 - Gemini API使用x-goog-api-key头部
            ...
            okHttpClient.newCall(okRequest).enqueue(new Callback() {
                ...
                public void onResponse(Call call, Response response) {
                    if (!response.isSuccessful()) {
                        callback.onError(new IOException("Gemini API调用失败: " + response));
                        return;
                    }
