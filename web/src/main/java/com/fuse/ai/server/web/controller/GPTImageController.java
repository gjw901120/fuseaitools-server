...
public class GPTImageController {
    ...
    public ResponseResult<?> v2TextToImage(@Valid @RequestBody GptImageV2TextToImageDTO request,
                                          @AuthenticationPrincipal UserJwtDTO userJwtDTO) {
        return ResponseResult.success(gptImageService.v2TextToImage(request, userJwtDTO));
    }

    @PostMapping("/v2-image-to-image")
    public ResponseResult<?> v2ImageToImage(@Valid @RequestBody GptImageV2ImageToImageDTO request,
                                          @AuthenticationPrincipal UserJwtDTO userJwtDTO) {
        return ResponseResult.success(gptImageService.v2ImageToImage(request, userJwtDTO));
    }
}