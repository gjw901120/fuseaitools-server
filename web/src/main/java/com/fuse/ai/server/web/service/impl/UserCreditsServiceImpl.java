...
public class UserCreditsServiceImpl implements UserCreditsService {
        ...
        if(PricingTypeEnum.CHARACTER.equals(model.getPricingType())) {
            ...
            if(model.getIsPricingRules()  == 1) {
                ModelsPricingRules pricingRules = switch (extraData.getType()) {
                    ...
                    case DURATION_QUALITY_SCENE ->
                            modelsPricingRulesManager.getDetailByModelIdAndDurationQualityScene(model.getId(), extraData.getDuration(), extraData.getQuality(), extraData.getScene());
                    case DURATION_SCENE ->
    ...
    @Override
    public verifyCreditsBO sseVerifyCredits(Integer userId, Models model, ExtraDataBO extraData, SseCallback callback) {
        /*
          1.判断用户余额是否充足
          2.token/character/duration类型 余额 >= 10 才可用
          3.once验证余额是否充足，包含折扣
        ...
        if(PricingTypeEnum.CHARACTER.equals(model.getPricingType())) {
            ...
            if(model.getIsPricingRules()  == 1) {
                ...
                if(pricingRules == null) {
                    SseBaseException.throwError(ResponseErrorType.CREDITS_IS_NOT_ENOUGH, "credits is not enough", callback);
                }
                pricingRulesId =  pricingRules.getId();
                ModelsPricingOnce modelsPricingOnce = modelsPricingOnceManager.getDetailById( pricingRules.getPricingId());
                credits =  modelsPricingOnce.getCredits();
